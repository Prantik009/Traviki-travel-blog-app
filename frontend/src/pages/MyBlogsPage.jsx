...
import { getMyPost } from "../store/slices/blogSlice";
...
export const MyBlogsPage = () => {
  ...
  const { myPosts, loading } = useSelector((state) => state.blog);
  // console.log("My posts are: ", myPosts);

  if (loading) return <>Loading...</>;
  if (myPosts.length<1) return (
    <div className="pt-20 bg-gray-100 w-full flex flex-col items-center justify-center h-[100vh] ">
       ...
       <p>Please <span className="text-blue-600 underline"><Link to="/add-posts">add post</Link></span></p>
    </div>
  )

  return (
    <div className="blogs py-4 place-items-center pt-10">
